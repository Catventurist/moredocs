  ...
  <UiDialog v-model:open="open">
    <UiDialogContent class="p-0">
      ...
      <UiCommand v-model:search-term="input" class="h-svh sm:h-[350px]">
        <UiCommandInput :loading="searchLoading" :placeholder="placeholderDetailed" @keydown.enter="handleEnter"
        ...
        <UiCommandList class="text-sm" @escape-key-down="open = false">
          ...
          <div v-else-if="searchResult?.length" class="p-1.5">
            <NuxtLink v-for="(item, i) in searchResult" :id="i" :key="item.id" :to="item.id"
              ...
              <span class="ml-2 self-center truncate text-xs text-muted-foreground"
