  ...
  <UiScrollArea orientation="vertical" class="relative h-full overflow-hidden py-6 pr-6 text-sm" type="hover">
    ...
    <ul v-if="config?.aside.useLevel" class="border-b mb-1 pb-4">
      <li v-for="link in navigation" :key="link.id">
        <NuxtLink v-if="link?.title || link?._path || link?.icon" :to="link._path"
          class="mb-1 flex w-full gap-2 rounded-md px-3 py-2 transition-all hover:bg-muted" :class="[
            path.startsWith(link._path) && 'bg-muted font-semibold text-primary hover:bg-muted',
          ]">
          <Icon :name="link.icon" class="self-center" size="16" />
          {{ link.title }}
        </NuxtLink>
      </li>
    </ul>
